...
const ParticleMap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const particleSpacing = Math.max(
      6,
      Math.floor(6 * (width / baseResolution))
    ); // Ajustar proporcionalmente

    // Cargar la imagen
    const image = new Image();
    image.src = `${path}/images/nocs-network/nocs.png`;
        ...
        for (let x = 0; x < width; x += particleSpacing) {
          ...
          if (a > 0 && Math.random() < dispersionFactor) {
            particles.push({
              ...
              alpha: isSmallScreen
                ? Math.random() * 0.6 + 0.1
                : Math.random() * 0.9 + 0.1, // Opacidad aleatoria entre 0.1 y 1
              size: isSmallScreen
                ? Math.random() * 1 + 0.8
                : Math.random() * 2 + 1, // Tamaño aleatorio
              ...
              isStatic: isSmallScreen
                ? Math.random() < 0.4
                : Math.random() < 0.2, // 20% de las partículas serán estáticas
      ...
      const drawParticles = () => {
        ...
        particles.forEach((particle) => {
          if (!particle.isStatic) {
            ...
            particle.x =
              particle.baseX +
              Math.sin(performance.now() * particle.speed * 0.002) *
                particle.offsetX;
            particle.y =
              particle.baseY +
              Math.cos(performance.now() * particle.speed * 0.002) *
                particle.offsetY;
          } else {
